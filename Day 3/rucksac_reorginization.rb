...
arr2 = Array.new(0)
  ...
  arr1.each_with_index do
    |v, i|
    arr2.push(arr1[i][(arr1[i].length / 2)...arr1[i].length])
    arr1[i] = arr1[i][0...(arr1[i].length / 2)]
  end
  compareSacks(arr1, arr2)
end

def compareSacks(arr1, arr2)
  arr3 = []
  arr1.each_with_index do
    |v, i|
    arr3.push(arr1[i].split('') & arr2[i].split(''))
  end
  scoreSacks(arr3)
end
...
##So if you can't set global variables to use in a method, you can pass one in, okay that's fine, but if you need to set those two variables to be used
#in other methods, how do you store them when they get returned? Also how do you return two items?
